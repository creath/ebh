...
uint256 hashGenesisBlock("0x00000af90f694092b23871bf7e2af5b56cf30858fde9c53e5812954460cefd57");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EBlackHoleCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EBlackHoleCoin Signed Message:\n";
...
{
    // EBlackHoleCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 232 * COIN;

    nSubsidy >>= (nHeight / 86206897);
	
	if (nHeight == 1)
		return 60000000000 * COIN;
...
{
    // EBlackHoleCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
		...
		const char* pszTimestamp = "ebh genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc4, 0xb3, 0xde }; // EBlackHoleCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EBlackHoleCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EBlackHoleCoinMiner : ProcessBlock, block not accepted");
    }
